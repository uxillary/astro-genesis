...
const HelpOverlay = ({ open, onClose }: HelpOverlayProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative w-[min(640px,90vw)] rounded-[28px] border border-white/15 bg-panel/85 p-8 font-mono text-[0.7rem] uppercase tracking-[0.24em] text-white shadow-panel">
        ...
        <div className="grid gap-4 text-left text-[0.65rem] uppercase tracking-[0.28em] text-mid">
          <p>Press <span className="text-white">C</span> for credentials overlay.</p>
          <p>Press <span className="text-white">?</span> to toggle this dossier help.</p>
        ...
        <button
          type="button"
          className="mt-8 block w-full rounded-full border border-white/20 px-4 py-3 text-[0.65rem] text-dim hover:text-white/90"
          onClick={onClose}
        >
          Return to console
        </button>
